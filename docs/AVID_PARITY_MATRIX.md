...
| Background services | Partial | Desktop ingest/export jobs are tracked and surfaced, ingest now records explicit per-surface editability (`native`, `proxy-only`, `mezzanine-required`, `adapter-required`, `unsupported`), managed media metadata, waveforms, best-effort proxies, and watch-folder scans. Real durable distributed processing and a full render farm are still absent. |
