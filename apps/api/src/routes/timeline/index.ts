...
router.post('/:timelineId/tracks', requireProjectAccess('EDITOR'), validate(schemas.createTrack), async (req: Request, res: Response) => {
...
router.patch('/:timelineId/tracks/:trackId', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
  const allowed = ['name', 'muted', 'locked', 'solo', 'volume', 'pan', 'color', 'height', 'sortOrder', 'isPatched'];
  const data: any = {};
  allowed.forEach((k) => { if (req.body[k] !== undefined) data[k] = req.body[k]; });

  const track = await db.track.update({ where: { id: req.params.trackId }, data });
  res.json({ track });
});

// DELETE /projects/:projectId/timelines/:timelineId/tracks/:trackId
router.delete('/:timelineId/tracks/:trackId', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
...
router.post('/:timelineId/clips', requireProjectAccess('EDITOR'), validate(schemas.createClip), async (req: Request, res: Response) => {
...
router.patch('/:timelineId/clips/:clipId', requireProjectAccess('EDITOR'), validate(schemas.trimClip), async (req: Request, res: Response) => {
...
router.delete('/:timelineId/clips/:clipId', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
  await db.clip.delete({ where: { id: req.params.clipId } });
  res.status(204).send();
});

// POST split clip at timecode
router.post('/:timelineId/clips/:clipId/split', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
  const { splitTime } = req.body;
  if (!splitTime) throw new BadRequestError('splitTime required');
...
router.post('/:timelineId/clips/:clipId/effects', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
...
// DELETE effect
router.delete('/:timelineId/clips/:clipId/effects/:effectId', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
...
router.delete('/:timelineId/markers/:markerId', requireProjectAccess('EDITOR'), async (req: Request, res: Response) => {
